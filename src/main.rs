...
fn main() {
    ...
    let slow_pair: sync::Arc<(sync::Mutex<HashMap<String, HashMap<String, String>>>, sync::Condvar)>
    ...
    let thr3 = std::thread::spawn(move || {
        let contents = fs::read_to_string("ZASILENIE.txt")
        ...
        let result = cvar.wait_while(mtx.lock().unwrap(), |data: &mut HashMap<String, Vec<String>>| data.len() == 0).unwrap();
    ...
    let thr2 = std::thread::spawn(move || {
        ...
        let contents = fs::read_to_string("BO.txt")
        .expect("Something went wrong reading the file");
        let mut lines =  contents.split("\r\n").collect::<Vec<&str>>();
        lines.remove(0);
        lines.remove(lines.len()-1);
    ...
    let thr1 = std::thread::spawn(move || {
        ...
        let contents = fs::read_to_string("SLOW.txt")
        .expect("Something went wrong reading the file");
        let mut lines =  contents.split("\r\n").collect::<Vec<&str>>();
        lines.remove(0);
        lines.remove(lines.len()-1);
    ...
    for line in &zasil_contents {
        ...
        clients.insert(tmp[0].to_string(), vec![tmp[2].to_string(), tmp[3].to_string(), tmp[4].to_string()]);
    }

    let (mtx, cvar) = &*slow_pair;
    let slow_data = cvar.wait_while(mtx.lock().unwrap(), |data: &mut HashMap<String, HashMap<String, String>>| data.len() == 0).unwrap();
    thr1.join().unwrap();

    let (mtx, cvar) = &*bo_pair;
    let bo_data = cvar.wait_while(mtx.lock().unwrap(), |data: &mut HashMap<String, String>| data.len() == 0).unwrap();
    thr2.join().unwrap();
    ...
    for (c, loginy) in map {
        ...
        if len < 3 {
            ...
            if byl {
                ...
                match bo_data.get(&c.clone()) {                                                         //stare
                    Some(symbol) => {
                ...
                tmp_vec.push(slow_data.get("5").unwrap().get(&c_vec[1]).unwrap().to_string());          //nowe
            ...
            tmp_vec.push(slow_data.get("20").unwrap().get(&c_vec[1]).unwrap().to_string());             //nowe
            tmp_vec.push(c_vec[2].clone());
